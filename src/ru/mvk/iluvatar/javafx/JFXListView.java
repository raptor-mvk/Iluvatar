...
public class JFXListView<EntityType> implements ListView<EntityType> {
	...
	@NotNull
	private VBox prepareRoot() {
		// 5.0 is empirically selected spacing
		@NotNull VBox result = new VBox(5.0);
		result.setPadding(new Insets(5.0));
		@NotNull HBox buttonsRow = new HBox(5.0);
		buttonsRow.setPadding(new Insets(5.0));
		buttonsRow.getChildren().addAll(addButton, editButton, removeButton);
		result.getChildren().addAll(tableView, buttonsRow);
		return result;
	}
